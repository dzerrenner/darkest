define(["jquery", "knockout", "lodash", "moment", "semantic", "knockout-amd-helpers", "text"], function($, ko, _, moment){
    ...
    var MainViewModel = function() {
        ...
        self.nav.subscribe(function(newValue) {
            // html5 history
            debug("changing path to", newValue.section + "/" + newValue.page);
            history.pushState(newValue, newValue.section + " - " + newValue.page, "#" + newValue.section + "/" + newValue.page);
        });

        self.nav_path = function(section, page) {
            var nav = self.nav();
            return (nav.section == section && nav.page == page);
        };

        // initialize navigation
        window.addEventListener("popstate", function(e) {
            // try to get section and page from state
            self.nav(e.state || {section: 'heroes', page: 'overview'});
        });
    ...
    $(document).ready(function() {
        ...
        $.when.apply($, calls).done( function() {
            ...
            viewmodel.nav({section: 'heroes', page: 'overview'});
